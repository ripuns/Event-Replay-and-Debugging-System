...
| `V005__add_replay_job_scope_and_result.sql` | `ALTER TABLE replay_jobs ADD COLUMN aggregate_type VARCHAR(100), ADD COLUMN result JSONB`. | Phase 6 additions to the pre-existing (schema-only until now) `replay_jobs` table: `aggregate_type` lets a job scope itself to "every aggregate of this type" rather than only the whole project or one specific `aggregate_id` (both already supported by the original V003 columns); `result` holds the JSON summary (`{aggregatesProcessed, snapshotsCreated, failures}`) a completed job reports — see `src/replay-jobs/README.md`. |
