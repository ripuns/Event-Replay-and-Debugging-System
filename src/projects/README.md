...
| File | Purpose | Why |
|---|---|---|
| `projects.controller.ts` (`ProjectsController`) | Three routes, all guarded: `POST /v1/organizations/:orgId/projects` (`ApiKeyGuard` + `OrganizationAccessGuard`) creates an additional project in an org the caller's key belongs to, minting its first key. `POST /v1/projects/:id/api-keys` (`ApiKeyGuard` + `ProjectAccessGuard`) mints an extra key for a project the caller already holds a key for. `GET /v1/projects/:id` (`ApiKeyGuard` + `ProjectAccessGuard`) fetches a project. Annotated with `@ApiTags('projects')`/`@ApiBearerAuth('api-key')` (controller-level) and per-route `@ApiOperation` for Swagger. | Every write/read is scoped by a guard before the handler runs — the controller itself does no auth logic, only delegates to `ProjectsService`/`ApiKeyService` and DTO validation. |
...
| `dto/create-project.dto.ts` (`CreateProjectDto`) | `class-validator` DTO: `name` (required string, max 255), with `@ApiProperty` for Swagger. | Validated by the global `ValidationPipe` before reaching the controller/service. |
