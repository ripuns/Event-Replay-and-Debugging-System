...
| `dto/create-api-key.dto.ts` (`CreateApiKeyDto`) | `class-validator` DTO: `name` (required string, max 255), with `@ApiProperty` for Swagger. | Used by `ProjectsController`'s `POST /v1/projects/:id/api-keys` route so key names go through the same global `ValidationPipe` as every other input, instead of an untyped inline body. |
